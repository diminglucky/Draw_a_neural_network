...
describe("cnn-classifier grammar", () => {
  it("compiles VGG16 as a scale-aware CNN narrative rather than a residual diagram", () => {
    const ir = vgg16CanonicalIr();
    ...
    expect(plan.primitives.filter((item) => item.kind === "tensor_volume")).toHaveLength(6);
  ...
  it("fails closed for an unresolved structure and propagates print/density intent into the preview plan", () => {
    ...
    const compactPlan = cnnClassifierGrammar.compilePlan(cnnClassifierGrammar.compileSemanticModel(ir, compact), compact);
    ...
    expect(detailedPlan.coordinateSpace.width).toBeGreaterThan(compactPlan.coordinateSpace.width);
  });

  it("expands the deterministic page before a wider CNN can collide or overflow", () => {
    const base = vgg16CanonicalIr();
