...
describe("Drawing Run coordinator", () => {
  ...
  it("commits a concurrent command and its event exactly once", async () => {
    ...
    await expect(coordinator.listEvents("owner-1", started.runId)).resolves.toMatchObject([{ revision: 1, action: "received" }]);
  });
  ...
  it("uses a shared lease to fence recovery across coordinator instances", async () => {
    const foundation = new InMemoryFoundationStore();
