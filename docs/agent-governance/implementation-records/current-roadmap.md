...
| 当前路线图 | `in_progress` | M2.6 是唯一正式可执行节点；R0–R5 已有明确 ledger 映射，尚未改变任何节点 accepted 状态 |
| API | `passed` | `npm run api:check` 通过；完整 API 套件为 102 个测试文件、628 个测试通过 |
| Worker 单元测试 | `passed` | 234 通过、2 跳过、0 失败；跳过项不是真实 Visio 验收 |
| 通用未知网络直绘 | `in_progress` | UGS 严格合同、未知模块直绘、重复单元、candidate topology、非 feedback 闭环拒绝、feedback 回退为 candidate、结构证据强制、确定性 evidence 排序、ArchitectureIRv3 兼容投影、碰撞安全的多来源 evidence 保真、General Publication Graph，以及新建的通用 Figure Plan（只由 rank/lane 产生受限文档单位布局、语义连接器和 source/evidence mapping）均有单元和全套 API 证据；尚未接入公开 preview/Visio 路径 |
...
| R1 | UGS、General Publication Graph 与通用 Figure Plan | `in_progress` → `M2.6` | `UniversalGraphSpec`、v3-to-UGS adapter、General Publication Graph 与零模板双流融合、重复自定义模块、candidate edge/evidence region、非 feedback cycle、feedback candidate 回退、结构 evidence 强制、确定性 evidence 排序、碰撞安全的多来源 evidence fixture，以及无 renderer/Visio/Worker 指令的 Generic Figure Plan 已实现；通用 Figure Plan 只以 graph `layoutOrder` 生成受限布局、语义 connectors 和 source/evidence mappings，并拒绝 candidate、blocking topology、feedback 和重复 source/evidence 引用；聚焦 18 项与完整 API 634 项通过 | 独立审查后，才可开始 M2.5 的通用不可变 snapshot binding；不得提前 accepted |
...
| `npm run api:test -- apps/api/tests/universal-graph-spec.test.ts apps/api/tests/universal-graph-spec-adapter.test.ts apps/api/tests/general-publication-graph.test.ts apps/api/tests/general-publication-figure-plan.test.ts` | 4 个测试文件、22 个测试通过 | 是，R1 合同、通用语义图与 Figure Plan |
| `npm run api:test` | 103 个测试文件、634 个测试通过 | 是，R1 API 回归 |
...
R0 已恢复 legacy VGG16 夹具，M2.6 的 UGS/General Publication Graph 合同已补齐重复单元、candidate evidence region、非 feedback 闭环拒绝、feedback candidate 回退、结构 evidence 强制、确定性 evidence 排序和碰撞安全的多来源 evidence 保真；其通用 Figure Plan 也已补齐，只将语义图编成确定性受限布局和语义 connector，而不产生 renderer、Worker 或 Visio 指令。M2.6 仍未 accepted，也没有通用公开 preview、immutable snapshot 或通用 Visio 输出。下一步做独立审查；只有审查通过后才开始 M2.5 Snapshot。PatternLibrary、输入适配器和 Worker 仍按各自节点后置。任何后续实现必须更新本记录、对应 baseline 和 Operation History，并根据证据决定是否更新 ledger。
