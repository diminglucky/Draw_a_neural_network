...
- [ ] Add failing tests for valid Ed25519 signatures, wrong-message rejection, malformed key rejection, and bounded base64 signatures.
- [ ] Run `npm run api:test -- apps/api/tests/device-proof.test.ts` and observe the missing verifier failure.
- [ ] Implement `verifyDeviceSignature(publicKeyPem, challenge, signatureBase64): boolean` using Node `crypto.verify` and add `DEVICE_PROOF_REQUIRED`, `DEVICE_PROOF_INVALID`, and `DEVICE_CHALLENGE_INVALID` error codes.
- [ ] Run focused tests and `npx tsc --noEmit`.
- [ ] Commit `feat: add device proof verifier`.
...
- [ ] Add failing tests for challenge creation and atomic consume, including second-consume rejection and expiry rejection.
- [ ] Run focused tests and observe missing challenge methods/table contract.
- [ ] Add `DeviceChallenge`, `createDeviceChallenge`, and `consumeDeviceChallenge` to both stores; use PostgreSQL `UPDATE ... WHERE consumed_at IS NULL AND expires_at > $2 RETURNING`.
- [ ] Add the initial schema table and an idempotent upgrade migration for existing Docker volumes.
- [ ] Run Store tests and TypeScript checking.
- [ ] Commit `feat: persist device challenges`.
...
- [ ] Add failing tests for challenge issuance, proof login, replay rejection, wrong-device rejection, and production missing-proof rejection.
- [ ] Run focused tests and confirm routes/SessionService have no challenge flow.
- [ ] Implement `createLoginChallenge`, consume-before-verify proof validation, `REQUIRE_DEVICE_PROOF`, `/api/auth/challenge`, and login payload parsing.
- [ ] Preserve development test login without proof only when `requireDeviceProof` is false.
- [ ] Run all API tests and TypeScript checking.
- [ ] Commit `feat: require device proof for production login`.
...
- [ ] Add failing tests for IPC identity/signing delegation, missing bridge rejection in production, and development-only browser fallback.
- [ ] Implement a provider that accepts an injected bridge; renderer code can request identity/signature but cannot access private key material.
- [ ] Keep the existing bootstrap path only behind an explicit development flag and document that it is not commercial security.
- [ ] Run client tests and JavaScript syntax checks.
- [ ] Commit `feat: add electron device key provider boundary`.
...
- [ ] Extend PostgreSQL smoke to create, consume, reject replay, and reject expired challenges.
- [ ] Run all API tests, both smoke scripts, source checks, syntax checks, and Docker health checks.
- [ ] Record that DPAPI native implementation and signed installer acceptance remain pending.
