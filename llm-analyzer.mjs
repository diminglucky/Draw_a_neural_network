...
const SYSTEM_PROMPT = [
  ...
  "3. A compound module explicitly present in the input is drawn as a SINGLE labeled color block.",
  "   Do NOT expand unobserved internal layers or emit attributes.internalGraph. Set family",
  "   \"custom\", compoundKind \"module\", and preserve the observed module label. Decompose a",
  "   module only when its internal operators and connections are explicitly provided as evidence.",
  ...
  "   Model names are only retrieval and disambiguation hints, not topology evidence.",
  "   Never fill in nodes, edges, groups, or",
  "   dimensions from a familiar architecture name alone.",
  ...
  "11. When the evidence explicitly identifies high-level sections, emit a top-level \"groups\"",
  "    array that DRIVES THE LAYOUT:",
  "    [ { \"id\": string, \"label\": string, \"kind\": string, \"nodeIds\": [ node ids ] } ].",
  "    THE ARRAY ORDER IS THE LEFT-TO-RIGHT COLUMN ORDER stated or shown by the input. Within a",
  "    group, nodes are auto-arranged by feature-map resolution, largest on top. \"kind\" is a",
  "    free label used only for the legend/color, never a layout directive. Every grouping uses",
  "    the same generic grid and must be supported by input evidence.",
  "    Each node belongs to at most one group. If no clear grouping exists, omit groups.",
  "12. Preserve explicitly observed branch structure, recurrent or memory state flow, and named",
  "    input/output ports. Represent them with ordinary nodes and edges; do not infer a branch,",
